...
\def\vref>#1>{la~\refsc{#1}, pá\-gi\-na~\refpg{#1}}
\def\href>#1>{la~\refsc{#1}}
...
\def\secid{>}
\dest
\vbox to22pt{\centerline{\fontone Referencias}\toc1{Referencias}\vss}
...
\dest
\vbox to22pt{\centerline{\fontone Índice temático}\toc1{Índice temático}\vss}
\vskip 1pc plus1pc minus6pt
...
\dest
\vbox to22pt{\centerline{\fontone Índice}\toc1{Índice}\vss}
...
\def\toclineerror#1#2#3#4{\message{wrong tocline: #1, #2, #3, #4}%
 \ignorespaces}
